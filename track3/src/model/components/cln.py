...
class LayerNormLSTM(LSTMcell):
    ...
    def __init__(self,
        ...
        if ln_preact:
            if self.cln:
                self.ln_i2h = CLN(4*hidden_size, image_size)
                self.ln_h2h = CLN(4*hidden_size, image_size)
            else:
        ...
        if self.cln:
            self.ln_cell = CLN(hidden_size, image_size)
        else:
...
if __name__ == '__main__':
    ...
    image = Variable(Tensor(32, 1, 256)).squeeze(1)
    h = model.init_hidden(32)
    output, (h, c) = model(x, None, image)
    print(h.permute(1, 0, 2).squeeze(1).shape)